...
socket.on('initialize', async (data) => { 
    ...
    socket.on('LoadSceneComplete', () => {
        ...
        if (sceneName && !SINGLE_PLAYER_SCENES.includes(sceneName)) {
            ...
            const otherPlayers = Object.values(gamePlayers).filter(
                p => 
                p.id !== userId && 
                p.currentSceneName === sceneName
            );
...
app.post('/auth/kakao', async (req, res) => {
    ...
    try {
        ...
        const userSql = `INSERT INTO users (user_id, nickname, email, profile_image) VALUES (?, ?, ?, ?) 
        ON DUPLICATE KEY UPDATE nickname = VALUES(nickname), email = VALUES(email), profile_image = VALUES(profile_image)`;
...
app.get('/playerData/:userId', async (req, res) => { 
    ...
    const playerDataQuery = `SELECT u.user_id as id, u.nickname, c.level, c.gold, c.position_x, c.position_y, 
    c.position_z, c.rotation_y, cs.currentHp, cs.maxHp, cs.experience AS exp, cs.speed, cs.defense, cs.damage
    FROM users u LEFT JOIN characters c ON u.user_id = c.user_id 
    LEFT JOIN characterstats cs ON c.character_id = cs.character_id WHERE u.user_id = ? LIMIT 1;`;
    try {
